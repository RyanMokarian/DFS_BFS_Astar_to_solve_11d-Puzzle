/* 
 * A-Star Search Algorithm to Solve 11d-puzzle with 2 heuristics:
...
public class PuzzleAStar {
	...
	// constructor
	public PuzzleAStar (Node root, Node goal, int MAX_DEPTH, String heuristic)
	{
	    this.root = root;
	    this.goal = goal;
	    ...
	    AStar_UsingPriorityQueue(root);
	}
	   //A-Star using Priority Queue
		public void AStar_UsingPriorityQueue(Node initialNode)
		{
			//Assign cost to the Initial Node
				...
				closed_list.add(currentNode.data);
					if (collectionHelper.equals(currentNode.data, goal.data)) {
				    	...
				    	System.out.println(fileContent);
				    		...
				    		String fileName="";
				    		if(heuristic=="Hamming_Distance") fileName = "puzzleAs-h1";
				    		if(heuristic=="Manhattan_Distance") fileName = "puzzleAs-h2";
				    		collectionHelper.writeToFile(fileContent, fileName);
				    	System.out.println();
				    	System.out.println("A-Star search with \""+heuristic+"\" heuristic gives us a solution at"
					...
					{		depth = currentNode.getDepth();
							List<Node> childrenOfCurrentNode = getChildren(currentNode, log, closed_list);
		...
		public List<Node> getChildren(Node currentNode, List<int[][]> log, List<int[][]> closed_list ) {
				  ...
				  if((zero_x+x[i]>=0) && (zero_y+y[i]>=0) && (zero_x+x[i]<copyArray.length) && (zero_y+y[i]<copyArray[0].length)){
					  ...
					  copyArray[zero_x][zero_y]= copyArray[zero_x+x[i]][zero_y+y[i]]; copyArray[zero_x+x[i]][zero_y+y[i]]=0;
						...
						if (!new CollectionHelper().contains(log,tempArray)) {
							...
							if (heuristic == "Hamming_Distance")
								tempNode.setCost(closed_list.size()+costHamming(tempNode, goal));
							if (heuristic == "Manhattan_Distance")
								tempNode.setCost(closed_list.size()+costManhattan(tempNode, goal));
							//
